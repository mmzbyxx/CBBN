...
def create_logger(cfg, cfg_name):    
    time_str = time.strftime('%Y%m%d%H%M')    #时间戳

    cfg_name = os.path.basename(cfg_name).split('.')[0]

    log_dir = Path("./saved")  / ('z' + '_' + time_str + '_' + cfg_name) / Path(cfg.log_dir)
    ...
    model_dir =  Path("./saved")  / ('z' + '_' + time_str + '_' + cfg_name) / Path(cfg.model_dir)
