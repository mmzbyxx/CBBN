...
from methods import mixup_data, mixup_criterion, GLMC_mixed, SimSiamLoss
...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.dataset == 'cifar10' or config.dataset == 'cifar100':
        model = getattr(resnet_cifar, config.backbone)()     #获取特征提取模型resnet_fe
        classifier = getattr(resnet_cifar, 'Classifier')(feat_in=64, num_classes=config.num_classes)    #获取分类器，提取特征数64，类别数
    ...
    elif config.dataset == 'cifar100':    #数据集加载
    ...
    # 原来论文数据加载器，数据集中定义torch.utils.data.DataLoader
    train_loader = dataset.train_instance
    val_loader = dataset.eval
    weighted_train_loader = dataset.weighted_train_loader
    ...
    criterion = nn.CrossEntropyLoss().cuda(config.gpu)       #交叉熵
    ...
    for epoch in range(config.num_epochs):
        ...
        if config.dataset != 'places':
            block = None
        # train for one epoch
        train(train_loader, weighted_train_loader,model, classifier, criterion, optimizer, epoch, config, logger, block)    #训练

        # evaluate on validation set
        acc1, ece = validate(val_loader, model, classifier, criterion, config, logger, block)   #计算精度
...
def train(train_loader, weighted_train_loader, model, classifier, criterion, optimizer, epoch, config, logger, block=None):
    ...
    progress = ProgressMeter(    #训练过程中显示训练进度实例
        len(train_loader),
    ...
    weighted_train_loader = iter(weighted_train_loader)     #遍历加权的x2,y2的数据加载器

    training_data_num = len(train_loader.dataset)        #获取训练数据集的样本总数
    end_steps = int(training_data_num / train_loader.batch_size)   #训练步数=总样本数除以每个批次的大小
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        input_org_1 = images[0]
        input_org_2 = images[1]
        target_org = target

        try:     #从加权训练数据加载器 self.weighted_train_loader 中获取下一个批次的数据
            input_invs, target_invs = next(weighted_train_loader)
        except:     #如果数据加载器中的数据耗尽，就重新初始化加载器并再次获取数据
            weighted_train_loader = iter(weighted_train_loader)
            input_invs, target_invs = next(weighted_train_loader)

        # 将权重输入数据拆分为两个部分，长度与原始数据一样
        # input_invs是长度为2的列表
        input_invs_1 = input_invs[0][:input_org_1.size()[0]]
        input_invs_2 = input_invs[1][:input_org_2.size()[0]]

        # 创建独热编码表示，根据类别权重进行加权
        one_hot_org = torch.zeros(target_org.size(0), config.num_classes).scatter_(1, target_org.view(-1, 1), 1)
        one_hot_invs = torch.zeros(target_invs.size(0), config.num_classes).scatter_(1, target_invs.view(-1, 1), 1)
        one_hot_invs = one_hot_invs[:one_hot_org.size()[0]]     #保持两组独热编码的长度一致，取前面一部分，长度为 one_hot_org 的长度
        target_invs = target_invs.cuda()
        ...
        if torch.cuda.is_available():
            images = [image.cuda(config.gpu, non_blocking=True) for image in images]
            target = target.cuda(config.gpu, non_blocking=True)

        alpha=config.alpha
        lam = np.random.beta(alpha, alpha)   #α越大，lam越大，越倾向于x1图像

        mix_x, cut_x, mixup_y, mixcut_y = GLMC_mixed(org1=input_org_1, org2=input_org_2,
                                                    invs1=input_invs_1,invs2=input_invs_2,
                                                    label_org=one_hot_org,label_invs=one_hot_invs)

        # output_1 是模型的主要输出，output_cb_1 是可能的辅助输出，z1 是模型输出的表示，p1 是预测的辅助输出
        output_1, output_cb_1, z1, p1 = model(mix_x, train=True)
        output_2, output_cb_2, z2, p2 = model(cut_x, train=True)
        # 计算对比损失
        contrastive_loss = SimSiamLoss(p1,z2) + SimSiamLoss(p2,z1)

        # print(output_1.size())
        output1 = classifier(output_cb_1)
        output2 = classifier(output_cb_2)
        loss_mix = criterion(output1, target)
        loss_cut = criterion(output2, target)
        balance_loss = loss_mix + loss_cut
        loss = balance_loss + 4 * contrastive_loss


        # if config.mixup is True:       #原始论文使用Mixup
        ...
        total_size = torch.cat(images, dim=0).size(0)
        losses.update(loss.item(), total_size)
        top1.update(acc1[0], total_size)
        top5.update(acc5[0], total_size)
